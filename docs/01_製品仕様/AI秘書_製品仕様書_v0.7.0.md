...
**最終更新日**: 2026-02-19
**バージョン**: 0.7.0 (Native Auth Update)

## 1. 概要
AI秘書 ("AI Secretary") は、エグゼクティブおよびプロフェッショナル向けの次世代型業務支援アシスタントです。
PC上の操作ログをローカルで解析し、ユーザーの「集中力」と「スケジュール」をリアルタイムに防御します。

v0.7.0では、**認証基盤の完全刷新** を行いました。これにより、従来の課題であった「1時間ごとのカレンダー更新停止」が解消され、より安定したバックグラウンド運用が可能になりました。
...
*   **コンテキスト認識 (Context Awareness)**
    ...
    *   **タイムゾーン認識強化**: `Asia/Tokyo` (JST) を強制適用。
