...
**最終更新日**: 2026-02-21
**バージョン**: 0.7.1 (Memory 2.0 Phase 1 Update)

## 1. 概要
AI秘書 ("AI Secretary") は、エグゼクティブおよびプロフェッショナル向けの次世代型業務支援アシスタントです。
PC上の操作ログをローカルで解析し、ユーザーの「集中力」と「スケジュール」をリアルタイムに防御します。

v0.7.0では、**認証基盤の完全刷新** および **Memory 2.0 (Phase 1) の自動化** を行いました。これにより、従来の課題であった「1時間ごとのカレンダー更新停止」が解消され、また1日の振り返り（Memory）が深夜2時に完全自動で生成されるようになりました。
...
*   **L1 中期記憶 (Daily Summary / Memory 2.0)**
    *   **完全自動生成 (v0.7.0 New)**: 毎日深夜 02:00 (JST) に Cloud Functions が自律的にログを解析し、前日の要約を生成・保存します。
    *   **JST タイムゾーン完全同期**: 日本時間での 00:00 〜 23:59 の活動を正確に捉えます。
    *   **三層記憶構造**: 将来の L2（長期アーカイブ）へ向けた基盤として Firestore に永続化されます。
