...
### 2.1. ダッシュボード (メイン画面)
*   **集中力モニタリング (Focus Widget) (v0.8.0 Simplified)**
    - 入力操作に基づき、現在の「集中スコア」をリアルタイム算出。
    - **UIの簡略化**: 詳細なメトリクス（KPM等）を非表示にし、スコアと状態のみを表示するミニマルデザインを採用。
    - 3段階の状態判定:
...
*   **VectorRAG (純粋意味検索) (v0.8.5 New)**
    *   **Firestore Vector Search**: 記憶データをベクトル化し、K-NN（最近傍）検索により関連する過去ログをミリ秒単位で直接抽出。
    *   **意味ベースの検索**: 日付が不明な「あの件」といった曖昧な質問に対し、セマンティック（意味的）な関連性から的確に回答。
    *   **コスト・速度最適化**: 従来の「1日ずつ遡ってスキャンする」非効率なループを排除し、一発で関連コンテキストを取得。Gemini 2.5 Flashの性能を最大限に引き出します。
    *   **回答文章の整合性維持**: 引用タグ `[Memory: YYYY-MM-DD]` を回答の末尾や適切な箇所に挿入し、情報のソースを明示。
