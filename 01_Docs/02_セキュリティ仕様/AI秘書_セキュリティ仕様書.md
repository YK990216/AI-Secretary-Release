...
**最終更新日**: 2026/02/18
**バージョン**: 0.6.5 (Gemini 2.5 Flash / Firestore Secured / Multi-Calendar Support)
...
*   **セッション管理**: Firebase SDKによるトークン管理。
    ...
    *   **Secure Token Refresh**:
        *   **現状 (v0.6.5)**: Googleのセキュリティポリシー（OAuth 2.0 for Native Apps）により、Electron Renderer環境ではリフレッシュトークンの永続化に制限があります。結果として、**1時間ごとの再認証（サイレント）** が機能しない場合があります。
        *   **対策**: v0.6.5ではエラーを警告に留める暫定対応を実施。
        *   **次期計画 (v0.7.0)**: Node.jsメインプロセスでの「ネイティブアプリフロー」へ移行し、完全な永続化を実現します。
